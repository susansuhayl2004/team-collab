...
describe('Security Headers', () => {
  ...
  it('sets X-Frame-Options or CSP frame-ancestors', async () => {
    const res = await request(app).get('/health');
    expect('x-frame-options' in res.headers || 'content-security-policy' in res.headers).toBe(true);
  });
...
describe('Teams API', () => {
  ...
  it('POST /api/teams → creates a team (201)', async () => {
    const res = await request(app).post('/api/teams').send({ name: 'Alpha Squad', description: 'Core dev team' });
  ...
  it('POST /api/teams/:id/members → 400 on invalid role', async () => {
    const res = await request(app).post(`/api/teams/${teamId}/members`).send({ userId: 'u99', role: 'overlord' });
    expect(res.status).toBe(400);
  });
...
describe('Tasks API', () => {
  ...
  it('POST /api/tasks → creates task (201)', async () => {
    const res = await request(app).post('/api/tasks').send({ title: 'Fix auth bug', priority: 'high', status: 'todo' });
    expect(res.status).toBe(201);
...
describe('Users API', () => {
  ...
  it('POST /api/users → upserts existing user (200)', async () => {
    const res = await request(app).post('/api/users').send({ uid: 'u123', name: 'Carol Updated', email: 'carol@test.com' });
    expect(res.status).toBe(200);
...
describe('AI API', () => {
  it('GET /api/ai/health → returns configured status', async () => {
  ...
  it('POST /api/ai/assist → 400 when prompt missing', async () => {
    const res = await request(app).post('/api/ai/assist').send({});
    expect(res.status).toBe(400);
    expect(res.body.success).toBe(false);
  });
  it('POST /api/ai/assist → 400 when prompt is empty string', async () => {
    const res = await request(app).post('/api/ai/assist').send({ prompt: '' });
    expect(res.status).toBe(400);
  });
  it('POST /api/ai/assist → 400 when prompt too long', async () => {
    const res = await request(app).post('/api/ai/assist').send({ prompt: 'x'.repeat(2001) });
    expect(res.status).toBe(400);
  });
  it('POST /api/ai/assist → 503 when API key not configured', async () => {
    const original = process.env.GEMINI_API_KEY;
    delete process.env.GEMINI_API_KEY;
    const res = await request(app).post('/api/ai/assist').send({ prompt: 'Hello TeamBot' });
    expect(res.status).toBe(503);
    expect(res.body.success).toBe(false);
    process.env.GEMINI_API_KEY = original;
  });
  it('POST /api/ai/summarize → 400 when messages missing', async () => {
    const res = await request(app).post('/api/ai/summarize').send({});
    expect(res.status).toBe(400);
  });
  it('POST /api/ai/summarize → 400 when messages is empty array', async () => {
    const res = await request(app).post('/api/ai/summarize').send({ messages: [] });
    expect(res.status).toBe(400);
  });
  it('POST /api/ai/summarize → 503 when API key not configured', async () => {
    const original = process.env.GEMINI_API_KEY;
    delete process.env.GEMINI_API_KEY;
    const res = await request(app).post('/api/ai/summarize').send({ messages: [{ senderName: 'Alice', content: 'Hello' }] });
    expect(res.status).toBe(503);
    process.env.GEMINI_API_KEY = original;
  });
});
...
describe('Cache Utility', () => {
  ...
  it('invalidatePrefix removes matching keys', () => {
    ...
    expect(cache.invalidatePrefix('msgs:t1')).toBe(2);
...
describe('Validation Utility', () => {
  ...
  it('validateRequiredString → null on valid input', () => { expect(validateRequiredString('hello', 'field')).toBeNull(); });
  it('validateRequiredString → error on empty string', () => { expect(validateRequiredString('', 'field')).toBeTruthy(); });
  it('validateRequiredString → error on too long', () => { expect(validateRequiredString('x'.repeat(10), 'f', 5)).toBeTruthy(); });
  it('validateRequiredString → error on non-string', () => { expect(validateRequiredString(123, 'field')).toBeTruthy(); });
  it('validateEnum → null when value is valid', () => { expect(validateEnum('todo', 'status', ['todo', 'done'])).toBeNull(); });
  it('validateEnum → error on invalid value', () => { expect(validateEnum('flying', 'status', ['todo', 'done'])).toBeTruthy(); });
  it('validateEnum → null when value is undefined', () => { expect(validateEnum(undefined, 'status', ['todo'])).toBeNull(); });
  it('validateEmail → null on valid email', () => { expect(validateEmail('test@example.com')).toBeNull(); });
  it('validateEmail → error on invalid email', () => { expect(validateEmail('not-an-email')).toBeTruthy(); });
  it('validateEmail → error on null', () => { expect(validateEmail(null)).toBeTruthy(); });
  it('assertValid → throws ValidationError on first failure', () => { expect(() => assertValid('Name is required', null)).toThrow(ValidationError); });
  it('assertValid → does not throw when all pass', () => { expect(() => assertValid(null, null)).not.toThrow(); });
  it('sanitiseString → trims whitespace', () => { expect(sanitiseString('  hello  ')).toBe('hello'); });
  it('sanitiseString → returns empty string for null', () => { expect(sanitiseString(null)).toBe(''); });
});
